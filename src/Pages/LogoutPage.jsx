...
import "./LogoutAnimation.css"; // CSS importieren
...
const LogoutPage = () => {
  ...
  useEffect(() => {
    // Ladebildschirm nach 3 Sekunden verschwinden lassen
    const timer = setTimeout(() => {
      setIsAnimationComplete(true); // Animation beenden und Ladebildschirm ausblenden
    }, 3000); // Warten für 3,5 Sekunden

    return () => clearTimeout(timer); // Aufräumen der Timer-Ressourcen
  }, []);

  useEffect(() => {
    setTimeout(() => {
      setIsLoggedOut(true); // Nach 2 Sekunden wird der Logout abgeschlossen
    }, 2000); // 2 Sekunden

    setTimeout(() => {
        localStorage.removeItem("token");
        localStorage.removeItem("email");
      // Optional: Redirect nach der Animation
        navigate("/");
    }, 6000); // Redirect nach 6 Sekunden
  }, [navigate]);
    ...
    <div className={`container2 ${isAnimationComplete ? "fade-out" : ""}`}>
      ...
      <div className={`loading-page ${isAnimationComplete ? "fade-out" : ""}`}>
        ...
        <div className="name-container">
          ...
          <span className="logout-name">Signing you out</span>
        </div>
      </div>
    </div>
  );
